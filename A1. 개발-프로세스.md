...
| **자동 업데이트** | electron-updater + GitHub Releases(electron-builder `publish: github`) → `limsunglyong/FocusDAW-Mastering` |
| **재사용 자산** | 노이즈 제거·STFT·3D 스펙트로그램은 `_refer/FocusSpectogram/src/audio`, `src/viz` 이식 |

> **확정 사항(2026-06-26):**
> - GitHub 저장소: `https://github.com/limsunglyong/FocusDAW-Mastering.git` (자동 업데이트·릴리스 대상)
> - 인코더: MP3 = lamejs, FLAC = libflac.js (확정)
> - 코드 서명: **초기에는 미서명으로 진행**. 미서명 설치 시 Windows SmartScreen 경고가 표시될 수 있으며, 빌드 구조는 서명 유무와 무관하므로 추후 OV 인증서 발급 시 빌드 설정에만 추가한다.
...
flowchart LR
    ...
    subgraph DSP["DSP 노드"]
        ...
        F2[Spectral EQ: 5밴드 Min-φ Biquad]
...
> **현재 진행 상태(2026-07-01):** Phase 0~9 구현, 자동 업데이트 end-to-end, Phase 10 기존 인수 시험과 후속 회귀 시험까지 통과했다. 후속 기능을 통합해 앱 버전을 **v0.11.0**으로 승격했으며 Windows 설치본 빌드를 진행한다. 상세 현황은 `A2. 앱개발.md`와 `A3. 시험.md`를 기준으로 한다.
...
- EQ 모드 **Min-φ 5밴드 Parametric / 9-Band Graphic** — 주파수 축 SR 연동(20Hz~Nyquist). Linear Phase/Dynamic 모드는 범위 제외.
...
- electron-builder NSIS 빌드 + `publish: github`, electron-updater로 기동 시 업데이트 확인 후 사용자 선택에 따라 다운로드/재시작
...
- **A3. 시험.md** : 각 Phase 완료 시 자동/수동 시험 항목·절차 추가(완료분은 시험-아카이브.md 이관)
- **A4. 수정요청.md** : 사용자 피드백/버그 수정 사이클 관리
