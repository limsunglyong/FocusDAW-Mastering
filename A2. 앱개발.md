...
### #2. 하단 Transport 패널 — ✅ 구현·사용자 확인 완료(v0.2.11~v0.2.13)
...
- **구성(완료)**: 웨이브폼(현재 선택 파일 sourceBuffer min/max peak)·클릭 탐색·Rewind/Play·Pause/Forward(±5s)·**볼륨(모니터 전용·Export 미반영)+스피커 mute(이전 볼륨 복원)**·rAF 재생헤드(현재시간 2배·상태색·breathing, 전체길이 1.5배·이탤릭).
  ...
  - 관련 보완: v0.2.12(창 높이 성장·F4·시간 폰트/상태색·볼륨 슬라이더·Space) / v0.2.13(창 가로폭 드리프트 수정·mute·슬라이더 높이·현재시간 normal). 상세 History 참조.
- **남은 분리 작업 → v0.2.14**: 구간반복(A/B Loop) — `source.loop`/`loopStart`/`loopEnd` live 설정 + loop 구간 wrap 재생헤드. store `loop({enabled,a,b})`·`setLoop`. 웨이브폼 드래그로 A/B 지정.

## 다음: Phase 2 — Pre Processing (목표 v0.3.0)
> #1(파일 lazy 로딩)·#2(Transport 패널) 완료(~v0.2.13). **다음 작업 = v0.2.14 구간반복(A/B Loop)**(#2의 남은 분리 작업, 위 #2 섹션 참조) → 이후 본 Phase 2 진행.
