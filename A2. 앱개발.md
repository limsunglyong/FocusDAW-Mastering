...
| 7 | v0.8.0 | Export·Batch 렌더(인코딩·태그) | **진행 중(착수)** — 마스터 체인 빌더 분리(7-A)·오프라인 정밀 렌더(7-B)·**WAV(RIFF) 인코더(7-C)**·Electron 파일 저장 IO(7-D)·Export UI(단일/배치·진행률·취소·아트워크·Destination, 7-F/7-G)로 **WAV end-to-end 동작**. *(MP3 320/FLAC 인코더+태그(7-E)·Preview↔Export 정합 시험(7-H)은 후속.)* |
...
### B. Phase 7 잔여 단계
- **7-E**: MP3 320(**lamejs**) · FLAC(**libflac.js**) 인코더 + ID3v2/Vorbis comment 태그 + 아트워크(APIC/PICTURE) 임베드. **npm 의존성 설치 필요**(착수 전 사용자 확인) → 설치 후 [exportRunner.ts](src/export/exportRunner.ts) `encodeMaster` 의 `ExportUnsupportedError` 분기를 실제 인코더로 교체, `SUPPORTED_EXPORT_FORMATS` 확장. (E1 결정 = WAV 먼저, 2026-06-28.)
