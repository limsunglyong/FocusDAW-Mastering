...
### #2. 하단 Transport 패널 — 확정 범위(2026-06-27 사용자 결정)
- Top bar "Transport" 메뉴 → 현재 창 아래(Footer 위) **In-flow 접이식** 패널 펼침/접힘(기본 접힘). store `transportOpen`.
- 기존 좌측 Play/Space와 **동일 transport 상태 공유**(isOriginalPlaying/isPreviewing).
- **v0.2.11 범위 — ✅ 구현 완료(History 참조), 사용자 수동 시험 대기**: 웨이브폼(현재 선택 파일 sourceBuffer min/max peak, 클릭 탐색)·Rewind·Play/Pause·Forward·**볼륨(모니터 전용·Export 미반영)**·`requestAnimationFrame` 재생헤드.
  - 엔진: `ensureContext`에 영속 **master GainNode**(dry/wet→master→destination, 그래프 재빌드와 무관) + `setVolume`; `seek(time)`(재생 중 해당 오프셋 재시작/일시정지 시 오프셋 갱신, buffer 참조 복원).
  - 스토어: `transportOpen`·`volume`(0~1, 기본 1)·`toggleTransport`·`setVolume`·`seekPreview(time)`·`skip(±sec)`.
  - 컴포넌트: `TransportPanel.tsx`(웨이브폼 canvas + 컨트롤). 웨이브폼은 선택 파일 디코딩 전이면 placeholder.
- **v0.2.12 범위(분리)**: 구간반복(A/B Loop) — `source.loop`/`loopStart`/`loopEnd` live 설정 + loop 구간 wrap 재생헤드. store `loop({enabled,a,b})`·`setLoop`. 웨이브폼 드래그로 A/B 지정.
- Rewind/Forward 기본 ±5초(미세결정, 변경 가능).

## 다음: Phase 2 — Pre Processing (목표 v0.3.0)
> 위 #1·#2(transport/UX) 완료 후 진행한다.
