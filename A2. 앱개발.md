...
## 다음: Phase 5 — Stereo (목표 v0.6.0)

> 현 [previewEngine.ts](src/audio/previewEngine.ts) Stereo 는 v0.2.0 의 **Width(M/S matrix)만** 구현. A1 Phase 5 스펙 기준 확장 필요.

1. **Width**(0~200% M/S Side 게인) — 구현됨(유지/검증).
2. **Reverb/Delay send**(0~30%) — Convolver/Delay 병렬 send 추가.
3. **Bass Mono**(+ Bass 60~300Hz 크로스오버) — 저역만 모노화(LR 크로스오버 → 저역 M only).
4. **Mono Compatibility** 토글 — 모노 합 청취.
5. **Correlation 미터 상시 표시**(on/off 무관, GOOD/CHECK/RISK + fold loss dB) — 측정 연결([compute.ts](src/desk/compute.ts) stereo viz 는 현재 절차적 → 실측 상관도로 전환 검토).
6. Phase 5 완료 시 **v0.6.0**(Minor) 승격.
