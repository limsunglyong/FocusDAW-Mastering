...
## ▶ 다음 작업(우선) — 세션(프로젝트) 저장/불러오기 (v0.9.0, 2026-06-29 착수)

> **결정(2026-06-29 사용자 확정)**: 저장 범위 = **체인 설정만 / Preset형**(파일 큐·곡별 denoise 맵 미포함). 버전 v0.9.0 으로 Minor 승격 완료(상수만).

### 저장 대상 (세션 JSON 스키마)
- `name`, `savedAt`, `appVersion`
- `vals` — I.Input(`bit`/`rate`/`normimp`) · II.Pre(`denoise` 토글·`fadein`·`fadeout`, **`noiseDepth`/`denoiseAmt` 제외**) · III.EQ(`preset`·f0~4/g0~4/q0~4) · IV.Dynamics(`multiband`/`low`/`mid`/`high`/`ratio`/`transient`/`exciter`) · V.Stereo(`width`/`reverb`/`delay`/`crossover`/`bassmono`/`mono`) · VI.Loudness(`ceiling`/`target`/`sat`/`limiter`/`tplimit`) · VII.Export(`album`/`artist`/`year`/`genre`/`format`)
- `enabled`(7섹션 Bypass) · `activeUserPresetIdx` · `lastActivePresetName` · `artworkDataUrl` · `exportDir`

### denoise(II.Pre) 처리 방침
곡별 자동 추천값(`QueueFile.noiseDepth`/`denoiseAmt`)은 세션에 넣지 않음. 세션엔 `pre.denoise` On/Off 토글만 포함. 세션 적용 시 각 곡 per-file 값과 자동 추천(`if (!f.denoiseRecommended)`)은 그대로 유지.

### 구현 순서
1. ~~버전 상수 승격(package.json·lock·version.ts)~~ → **완료**
2. 세션 직렬화 모듈(`src/session/session.ts`) — 저장/복원 키 화이트리스트, 직렬화/역직렬화.
3. 스토어 `saveSession()`/`applySession()` — applySession 은 undo 후처리(rate 무효화→syncPreviewParams→refreshDenoise→analyzePreSelected) 재사용.
4. Electron IPC(`session:list/save/load/delete`) + preload `sessionIO` + `global.d.ts` 타입. `userData/sessions/*.json`.
5. `#sessions` 해시 라우팅 + 세션 창(카드 그리드: 이름·저장일시·켜진 모듈 칩·EQ 프리셋·LUFS·Format/Rate/Bit). Save/Load 모드.
6. `Project ▸ Save Session / Open` 메뉴 연결.
7. lint·build·수동 시험(A3 MT-9.0).

## Phase 7 마무리 + 음질 이슈 (2026-06-28 중단 지점)
...
## 수정요청사항 해결 계획 (v0.8.9) — Help 메뉴를 Transport(F4) 우측 이동, 섹션별 Undo/Redo 및 Export 취소(Cancel) 기능 추가

### 1) 목표
- TitleBar 내 Help 메뉴를 Transport(F4) 우측에 배치하여 메인 메뉴 영역을 정렬하고 접근성을 높임.
- 마스터링 파라미터 및 모듈 활성 상태에 대해 Ctrl+Z, Ctrl+Y, Ctrl+Shift+Z를 활용한 섹션별(I~VII) 독립 Undo/Redo 기록 보관 및 복구 기능 지원. (Project 저장/불러오기 제외)
- 내보내기(Export) 진행 중 사용자가 원할 때 즉시 중단할 수 있는 취소(Cancel) 버튼을 진행률 오버레이에 제공.

### 2) 구현 내용
- **Help 메뉴 Transport(F4) 우측 이동**:
  - `TitleBar.tsx` 내의 메뉴 바 렌더링 영역에서 `Help` 메뉴를 `Transport(F4)` 버튼 뒤로 이동하여 나란히 렌더링되도록 함.
- **섹션별 Undo/Redo 구현**:
  - `appStore.ts` 내에 각 섹션(0~6)별로 최대 100개의 `UndoSnapshot` 히스토리를 보관하도록 `undoStacks`, `redoStacks` 구조 도입.
  - Undo/Redo 실행 시, 현재 활성화된 섹션의 `ModId`에 매핑되는 파라미터(예: `pre.`, `spectral.`, `dynamics.` 접두사를 가지는 `vals` 및 활성화 상태 `enabled[modId]`)만 부분 선택하여 복원.
  - 이를 통해 다른 탭/설정창의 값이 보이지 않는 상황에서 백그라운드로 원복되는 부작용을 원천 방지.
  - 슬라이더 드래그 시작 시점(`useKnob.ts`, `Viz.tsx`) 및 텍스트 필드 포커스 시점(`Controls.tsx`)에 `pushUndoSnap()`을 호출하여 하나의 완전한 변경 이력으로 묶음.
  - 드래그 및 타이핑 중 변경 데이터 전송 시에는 `skipUndo: true` 플래그를 통해 불필요한 스택 생성 방지.
- **Export 진행 중 취소 기능**:
  - 전체 화면을 가리는 `LoadingCard` 컴포넌트(EXPORTING 진행률 표시)에 `onCancel` 프로프와 취소(CANCEL EXPORT) 버튼을 도입.
  - 취소 버튼 클릭 시 `cancelExport()` 액션이 호출되어 `exportCancelled` 플래그를 `true`로 설정하고 배치 루프가 즉시 중단되도록 함.
  - **Cancel 피드백 및 UI 개선**:
    - 취소 시 출력되는 "Export cancelled." 안내 메시지가 우측 컨트롤 패널의 높이를 늘리며 레이아웃을 망가뜨리는 문제를 수정하기 위해, 이를 오른쪽 패널에서 숨기고 왼쪽 `ExportViz` 내부의 `ALBUM ARTWORK` 드롭존 및 `DESTINATION` 영역 전체의 아랫줄에 별도의 독립된 블록으로 노출되도록 배치.
    - 취소 버튼 클릭 시, 오버레이 카드의 타이틀 텍스트를 "EXPORTING"에서 "CANCELLING..."으로 전환하고, `dkblink` 애니메이션(0.6초 주기 infinite alternate)을 연동해 깜박이도록 구현. 중복 클릭 방지를 위해 취소 진행 중에는 취소 버튼을 숨김 처리.
- **인앱 매뉴얼 단축키 테이블화**:
  - `ManualWindow.tsx` 파일 내의 `tips` 섹션에 단축키 목록 및 조작 안내(Space 재생/일시정지, F4 패널 토글, Ctrl+Z 실행취소, Ctrl+Y/Ctrl+Shift+Z 다시실행, 노브 더블클릭 기본값 복원)를 SNR 추천표 스타일의 깔끔한 테이블 형식으로 정리하여 가독성을 개선하고 다국어(ko/en)를 지원하도록 구현.
