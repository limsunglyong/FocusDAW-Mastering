...
### #2. 하단 Transport 패널
- Top bar "Transport" 메뉴 → 현재 창 아래 패널 펼침/접힘.
- 구성: 웨이브폼(클릭 탐색)·Rewind·Play/Pause·Forward·구간반복(A/B)·볼륨.
- 엔진 추가분: 그래프 끝 **master GainNode**(볼륨, 모니터 전용·Export 미반영), `seek(offset)`(offset 재생 기반), 구간반복(`source.loop`+`loopStart/loopEnd`), 웨이브폼 peak(min/max) 추출, `requestAnimationFrame` 재생헤드.
- **결정 필요(미정)**: 볼륨 Export 반영여부(권장 미반영), 구간반복↔Preview A/B·Rate 변경 상호작용, 패널 기본 접힘.
