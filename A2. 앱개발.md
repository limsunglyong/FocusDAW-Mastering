...
| 버전 | 날짜 | 구분 | 내용 |
|------|------|------|------|
| v0.1.2 | 2026-06-26 | Phase 0 (Patch) | **프로토타입 대비 세부 UI 오차 수정.** I~VII 카드 선택 시 상세 아이보리 패널 상단에 표시되는 마름모 위치가 프로토타입보다 떨어져 보여 `DetailSheet` notch `top` 값을 조정(`-8` → `-2`). Input 상세의 NOW SELECTED 행에서 파일명 flex item 이 줄어들지 않아 `WAV/24-bit/Stereo/LUFS` 칩이 아이보리 영역을 초과하던 문제를 `minWidth: 0`, `flex: '1 1 auto'`, 칩 영역 `flex: '0 0 auto'` 로 수정. **검증**: Electron 캡처로 마름모 배치와 파일명/칩 영역 내부 수납 확인. |
