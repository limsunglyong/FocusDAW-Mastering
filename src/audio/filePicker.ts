...
export function openAudioFilePicker(opts?: { directory?: boolean }): Promise<File[]> {
  return new Promise((resolve) => {
    const input = document.createElement('input');
    input.type = 'file';
    input.multiple = true;
    if (opts?.directory) {
      // webkitdirectory: 폴더 전체(하위 포함) 선택 → Recursive 스캔 효과
    ...
    input.addEventListener('change', () => {
      ...
      // 폴더 스캔은 비오디오가 섞이므로 오디오만 필터.
      // 명시적 파일 선택은 미지원 파일도 그대로 전달 → 로딩 단계에서 에러를 보여준다(MT-5).
      finish(opts?.directory ? all.filter((f) => isAcceptedAudioFile(f.name)) : all);
    });
 ...
 *  - 폴더 드롭: webkitGetAsEntry 로 하위까지 재귀 순회하되 오디오만 수집
 * webkitGetAsEntry 는 드롭 이벤트 동안 동기적으로 호출해야 하므로 엔트리를 먼저 캡처한다.
 */
export async function audioFilesFromDataTransfer(dt: DataTransfer | null): Promise<File[]> {
  if (!dt) return [];
  ...
  if (entries.some((e) => e)) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isFile) {
        ...
        await collectDir(entry as FileSystemDirectoryEntry, out); // 폴더는 오디오만
      }
    }
    return out;
  }
...
async function collectDir(dir: FileSystemDirectoryEntry, out: File[]): Promise<void> {
  ...
  while (batch.length) {
    for (const ent of batch) {
      if (ent.isFile) {
        ...
        await collectDir(ent as FileSystemDirectoryEntry, out);
      }
    }
