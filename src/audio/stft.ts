// FocusDAW Mastering Desk v0.2.24 (Phase 2) - STFT 스펙트로그램 엔진
// _refer/FocusSpectogram/src/audio/stft.ts 이식.
...
/** 기본 분석 파라미터 (Phase 2 결정 P2-3: FFT 2048 / hop 512(75% overlap) / Hann) */
...
/** AudioBuffer 의 모든 채널을 모노로 평균한 Float32Array 를 반환한다. */
export function bufferToMono(buffer: AudioBuffer): Float32Array {
  const channels: Float32Array[] = [];
  for (let ch = 0; ch < buffer.numberOfChannels; ch++) channels.push(buffer.getChannelData(ch));
  return mixToMono(channels, buffer.length);
}
