// FocusDAW Mastering Desk v0.2.26 (Phase 2) - 3D 좌표축 라벨 + 눈금
// _refer/FocusSpectogram/src/viz/axes.ts 이식(트림: LUFS 기준선/라벨 제거).
// 축: X=TIME(s), Z=FREQUENCY(Hz), Y=LEVEL(dB). 텍스트는 canvas 텍스처 스프라이트(항상 카메라를 향함).
...
const TITLE_TEXT_H = TICK_TEXT_H;
...
export function buildAxes(dims: AxisDims, ranges: AxisRanges) {
  ...
  const makeLabel = (text: string, color: string, textH: number, boxed = false): THREE.Sprite => {
    ...
    if (boxed) {
      ...
      ctx.fillStyle = 'rgba(3, 23, 22, 0.72)';
      ctx.fill();
      ctx.lineWidth = 3;
  ...
  addLine([new THREE.Vector3(-WIDTH / 2, 0, -DEPTH / 2), new THREE.Vector3(-WIDTH / 2, HEIGHT, -DEPTH / 2)], 0xe2f3f1);
  {
    ...
    const title = makeLabel('LEVEL (dB)', COLOR_INTENSITY, TITLE_TEXT_H, true);
