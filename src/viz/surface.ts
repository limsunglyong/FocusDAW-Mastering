// FocusDAW Mastering Desk v0.2.26 (Phase 2) - 스펙트로그램 → 3D 서피스 지오메트리
// _refer/FocusSpectogram/src/viz/surface.ts 이식(트림: LUFS/noise 하이라이트 옵션 제거, colormap 경로만 유지).
// 대용량 대비 시간축 max-pooling 다운샘플 + 주파수축 다운샘플.
// 축 매핑: X=시간, Z=주파수(깊이), Y=레벨(높이).
import * as THREE from 'three';
import type { Spectrogram } from '../audio/stft';
import { colormap } from './colormap';

/** 시간축 최대 프레임 수. 초과 시 구간별 max-pooling 병합. */
...
/** 서피스 월드 치수 (P2D-3: 시간/주파수 영역 60% 축소 → WIDTH/DEPTH 0.6배) */
const WIDTH = 60; // X (시간)  원본 100 → 60
const DEPTH = 36; // Z (주파수) 원본 60 → 36
...
/** 스펙트로그램으로부터 정점 변위 + 정점 색상(colormap) 서피스 지오메트리를 생성 */
export function buildSpectrogramGeometry(spec: Spectrogram): SurfaceBuildResult {
  ...
  for (let c = 0; c < cols; c++) {
    ...
    for (let r = 0; r < rows; r++) {
      const idx = c * rows + r;
      // 주파수축 방향 — 0Hz(r=0)를 z=-DEPTH/2, 고주파를 z=+DEPTH/2
