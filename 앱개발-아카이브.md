...
> [A2. 앱개발.md](A2.%20앱개발.md) 의 버전별 개발 이력 중 **Phase 0~8(v0.1.0 ~ v0.8.9)** 완료분을 이관·보관한다. (A2 지침 #3)
> 최근 이력(v0.9.x~)과 설계서·버그·다음 작업은 A2 본문에 유지.

| 버전 | 날짜 | 구분 | 내용 |
|------|------|------|------|
| v0.8.9 | 2026-06-29 | Patch | **Help 메뉴 우측 이동 · 섹션별 Undo/Redo · Export 취소.** ① Help 메뉴를 타이틀바 우측 끝(Transport(F4) 우측)으로 이동(가장자리 잘림 방지). ② 마스터링 파라미터·모듈 활성 상태 전역 Undo/Redo(Ctrl+Z·Ctrl+Y·Ctrl+Shift+Z) — 슬라이더 드래그/텍스트 입력 중에는 skipUndo 로 스택 그룹화. ③ Export 진행 오버레이에 CANCEL 버튼(cancelExport → exportCancelled 플래그, "CANCELLING…" dkblink). ④ 인앱 매뉴얼 단축키 테이블 추가(다국어). **검증:** lint·build 통과. (A3 MT-8.9 사용자 PASS) |
...
# 부록 — 완료된 계획/설계 상세 (Phase 0~8, 2026-06-29 이관)

> 아래는 A2 "다음 작업" 섹션에 누적되어 있던 **완료 Phase(0~8)의 계획·설계 상세·완료 수정요청 계획**이다. 구현은 코드와 [A2 설계서](A2.%20앱개발.md) 본문이 최신 기준이며, 본 부록은 의사결정 맥락 보존용이다.
